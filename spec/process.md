...
Chip Design Process (a.k.a cdp) is a package and uses the process.toml to store the process meta information.

## Process MetaInfo

process.toml must be located at the root of the process folder.

### name

The name of the process package. **Required**

This should be a valid name as defined by `^([A-Z0-9]|[A-Z0-9][A-Z0-9._-]*[A-Z0-9])$`.
...
The version of the process package. **Required**

This should be a valid [Semantic Versioning](https://semver.org/) string.
...
A short description of the process package. **Required**
...
The license of the process package.
...
The maintainers of the process package. **Optional**
...
A path, or list of paths corresponding to the README file(s) of the process package. **Optional**

The file(s) can be of any format, but markdown is preferred.

The contents of the README file(s) are used to populate the description
field of your distribution's metadata (similar to `long_description` in other tools).
...
An URL to the website of the process project. **Optional**
...
An URL to the repository of the process project. **Optional**
...
An URL to the documentation of the process package. **Optional**
...
One process is configured to look for dependencies in current environment (variables).
...
If you publish your package on web, they will appear in the `Process Links` section.
